...
1. Name of the first table (always plural): `products` 
...
1. Can one product have many orders? YES
2. Can one order have many products? YES
```
...
Join table for tables: products and orders
...
CREATE TABLE products_orders (
  product_id int,
  order_id int,
  constraint fk_product foreign key(order_id) references orders(id) on delete cascade,
  constraint fk_order foreign key(product_id) references products(id) on delete cascade,
